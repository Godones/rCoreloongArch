use core::arch::asm;
use crate::config::{
    IOCSR_IPI_SEND_BLOCKING, IOCSR_IPI_SEND_CPU_SHIFT, IOCSR_MBUF_SEND_BLOCKING,
    IOCSR_MBUF_SEND_BOX_SHIFT, IOCSR_MBUF_SEND_BUF_SHIFT, IOCSR_MBUF_SEND_CPU_SHIFT,
    IOCSR_MBUF_SEND_H32_MASK, LOONGARCH_IOCSR_IPI_SEND,
...
pub fn ipi_write_action(cpu: usize, action: u32) {
    ...
    for i in 0..32 {
        if action.get_bit(i) {
            ...
            val |= (1 << i) as u32;
            println!("ipi_write_action: val = {:#x}", val);
